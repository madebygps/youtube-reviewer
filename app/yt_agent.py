...
class ActionableInsight(BaseModel):
    description: str = Field(..., description="The actionable insight derived from the video captions.")
    timestamp: str = Field(..., description="The timestamp from when the actionable insight is based off of.")

class ActionableInsightsAgentResponse(BaseModel):
    insights: List[ActionableInsight] = Field(..., description="A list of actionable insights derived from the video captions.")
...
class CaptionExtractor(Executor):
    ...
    async def handle(self, message: ChatMessage, ctx: WorkflowContext[str]) -> None:
        ...
        if custom_prompt:
            await ctx.send_message(f"{srt_captions}|||CUSTOM_PROMPT|||{custom_prompt}")
        else:
...
class ActionableSummaryGenerator(Executor):
    ...
    @handler
    async def handle(self, message: str, ctx: WorkflowContext[List[ActionableInsight]]) -> None:
        ...
        if custom_prompt:
            ...
            prompt = (
                "Based on the following YouTube video captions, generate a concise "
                "summary highlighting key points and actionable insights for the content creator:\n\n"
                f"{captions}"
            )
